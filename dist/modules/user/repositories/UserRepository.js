        ...
        mysql_1.pool.getConnection((err, connection) => {
            (0, bcrypt_1.hash)(password, 10, (err, hash) => {
                ...
                connection.query('INSERT INTO users (user_id, name, email, password) VALUES (?,?,?,?)', [(0, uuid_1.v4)(), name, email, hash], (error, result, fields) => {
                    ...
                    response.status(200).json({ message: 'User sucessfully created' });
                });
            });
        });
    }
    login(request, response) {
        ...
        mysql_1.pool.getConnection((err, connection) => {
            connection.query('SELECT * FROM users WHERE email = ?', [email], (error, results, fields) => {
                connection.release();
                if (error) {
                    return response.status(400).json({ error: "Error in your authentification!" });
                }
                (0, bcrypt_1.compare)(password, results[0].password, (err, result) => {
                    ...
                    if (result) {
                        ...
                        return response.status(200).json({ token: token, message: 'Sucessfully authenticated' });
                    }
                });
            });
        });
