...
exports.VideoRepositery = void 0;
const mysql_1 = require("../../../mysql");
const uuid_1 = require("uuid");
class VideoRepositery {
    create(request, response) {
        ...
        mysql_1.pool.getConnection((err, connection) => {
            connection.query('INSERT INTO videos (video_id, user_id, title, description) VALUES (?,?,?,?)', [(0, uuid_1.v4)(), user_id, title, description], (error, result, fields) => {
                ...
                response.status(200).json({ message: 'Video sucessfully created' });
            });
        });
    }
    getVideos(request, response) {
        ...
        mysql_1.pool.getConnection((err, connection) => {
            connection.query('SELECT * FROM videos WHERE user_id = ?', [user_id], (error, results, fields) => {
                connection.release();
                if (error) {
                    return response.status(400).json({ error: 'Error to find the videos!' });
                }
                return response.status(200).json({ message: 'Videos returned sucessfully!', videos: results });
            });
        });
    }
...
exports.VideoRepositery = VideoRepositery;
